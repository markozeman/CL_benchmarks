...
class NormalNN(nn.Module):
    ...
    def __init__(self, agent_config):
        ...
        if agent_config['gpuid'][0] >= 0:
            ...
            self.gpu = False
        self.init_optimizer()
        # self.reset_optimizer = False    # todo
    ...
    def validation(self, dataloader, is_test=None):
        ...
        for i, (input, target, task) in enumerate(dataloader):
            ...
            auroc = accumulate_auroc(output, target, auroc)
            auprc = accumulate_auprc(output, target, auprc)
        ...
        if is_test is None:
            ...
            self.log(' * Test Acc {acc.avg:.3f}, Total time {time:.2f}'.format(acc=acc, time=batch_timer.toc()))

        return acc.avg, auroc.avg, auprc.avg
    ...
    def learn_batch(self, train_loader, val_loader=None, do_early_stopping=False, stopping_criteria='acc'):
        ...
        for epoch in range(1, num_epochs+1):
            ...
            self.model.train()
            # self.scheduler.step(epoch)  # todo
            ...
            self.scheduler.step(acc.val)  # todo: added after optimizer.step()

            # Evaluate the performance of current task
            if val_loader != None:
                val_acc, val_auroc, val_auprc = self.validation(val_loader)

            if do_early_stopping:
                if stopping_criteria == 'acc':
                    if val_acc >= best_prev_value:
                        best_prev_value = val_acc
                    else:
                        print('Early stopped - %s got worse in this epoch.' % stopping_criteria)
                        break
                elif stopping_criteria == 'auroc':
                    if val_auroc >= best_prev_value:
                        best_prev_value = val_auroc
                    else:
                        print('Early stopped - %s got worse in this epoch.' % stopping_criteria)
                        break
                elif stopping_criteria == 'auprc':
                    if val_auprc >= best_prev_value:
                        best_prev_value = val_auprc
                    else:
                        print('Early stopped - %s got worse in this epoch.' % stopping_criteria)
                        break
        return epoch
...
def accumulate_auroc(output, target, meter):
    # only for single-headed model
    _, auroc, _ = get_stats(output['All'], target)
    meter.update(auroc * 100, len(target))
    return meter


def accumulate_auprc(output, target, meter):
    # only for single-headed model
    _, _, auprc = get_stats(output['All'], target)
    meter.update(auprc * 100, len(target))
    return meter


def get_stats(outputs, target):
